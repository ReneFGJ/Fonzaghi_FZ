 ...
 */
	...
	function busca_consultora()
			...
			array_push($cp,array('$S7','','C�digo consultora',True,False));
	...
	function senha_muda($old,$new,$lem)
		{
			if (trim($old) != trim($this->cl_senha))  
				{ return('Senha original inv�lida'); }
	...
	function mostra_dados_pessoais2()
		...
		function mostra_dados_pessoais_completo($id)
		{
			...
			$sx .= '	<fieldset><legend>Endere�o</legend>';
	...
	function mostra_endereco(){
			...
			$sx .= '		<TR class="lt0">'.chr(13);
			$sx .= '		<TD colspan="2" align="center">'.$link1.'::Novo Endere�o::</a></TD>'.chr(13);
	...
	function pecas_retiradas(){
			...
			$sx = '<TABLE width="<?=$tab_max;?>" align="center" border="0">
					...
					<TD width="20%"><fieldset><legend>Joias</legend><center><font class="lt2">'.$ac[0].'<BR>&nbsp;'.$ad[0].'&nbsp;<BR><fonts class="lt0">limite&nbsp;'.$lm[0].'&nbsp;pe�as(s)<BR>R$ '.$vm[0].'</center></TD>
					<TD width="20%"><fieldset><legend>Modas</legend><center><font class="lt2">'.$ac[1].'<BR>&nbsp;'.$ad[1].'&nbsp;<BR><fonts class="lt0">limite&nbsp;'.$lm[1].'&nbsp;pe�as(s)<BR>R$ '.$vm[1].'</center></TD>
					<TD width="20%"><fieldset><legend>�culos</legend><center><font class="lt2">'.$ac[2].'<BR>&nbsp;'.$ad[2].'&nbsp;<BR><fonts class="lt0">limite&nbsp;'.$lm[2].'&nbsp;pe�as(s)<BR>R$ '.$vm[2].'</center></TD>
					<TD width="20%"><fieldset><legend>Cat�logo</legend><center><font class="lt2">'.$ac[3].'<BR>&nbsp;'.$ad[3].'&nbsp;<BR><fonts class="lt0">limite&nbsp;'.$lm[3].'&nbsp;pe�as(s)<BR>R$ '.$vm[3].'</center></TD>
					<TD width="20%"><fieldset><legend>Sensual</legend><center><font class="lt2">'.$ac[4].'<BR>&nbsp;<?'.$ad[4].'&nbsp;<BR><fonts class="lt0">limite&nbsp;'.$lm[4].'&nbsp;pe�as(s)<BR>R$ '.$vm[4].'</center></TD>
					<TD width="20%"><fieldset><legend>Express Modas</legend><center><font class="lt2">'.$ac[5].'<BR>&nbsp;'.$ad[5].'&nbsp;<BR><fonts class="lt0">limite&nbsp;'.$lm[5].'&nbsp;pe�as(s)<BR>R$ '.$vm[5].'</center></TD>
					<TD width="20%"><fieldset><legend>Express Joias</legend><center><font class="lt2">'.$ac[6].'<BR>&nbsp;'.$ad[6].'&nbsp;<BR><fonts class="lt0">limite&nbsp;'.$lm[6].'&nbsp;pe�as(s)<BR>R$ '.$vm[6].'</center></TD>
					</TABLE>
					';
	...
	function atualizar_email_auto($id='',$email='')
	{
		...
		if($line=db_read($rlt))
		{
			return('J� existe este e-mail no banco de dados');
	...
	function lista_aniversariantes($data)
	{
		...
		$rlt = db_query($sql);
		$sx = '<table width="100%">';
		$sx .= '<tr><th class="tabelaTH">C�digo</th><th class="tabelaTH">Consultora</th></tr>';
		while($line = db_read($rlt))
		{
	...
	function cp_password()
	{
		...
		/*1*/array_push($cp,array('${','','Senha',False,True));
       	/*2*/array_push($cp,array('$S7','','C�digo da cliente',True,True));
	...
	function valida_password($cliente, $pass,$nw_pass,$nw_pass2,$nw_lembrete)
	    {
	       ...
	       if($psw_old!=$pass){        $msg .="Senha antiga n�o confere";}
           if($nw_pass!=$nw_pass2){    $msg.="<BR>Nova senha digitada n�o confere com a redigitada";}
           if(!(strlen($nw_pass)==4)){ $msg.="<BR>Senha necessita de 4 digitos";}
           if($psw_old==$nw_pass){     $msg.="<BR>Senha n�o pode ser igual a antiga";}
